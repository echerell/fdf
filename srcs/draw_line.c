...
/*   Updated: 2022/01/20 22:31:20 by echerell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	(data->img_addr)[p1.x_t * 4 + data->line_len * p1.y_t] = color & 0xFF;
	(data->img_addr)[p1.x_t * 4 + data->line_len * p1.y_t + 1] =
		(color >> 8) & 0xFF;
	(data->img_addr)[p1.x_t * 4 + data->line_len * p1.y_t + 2] =
		(color >> 16) & 0xFF;
		...
		t_grad grad)
{
	...
	while (p1.x_t <= p2.x_t)
	{
		...
		if (0 <= p1.x_t && p1.x_t < WIN_WIDTH && 0 <= p1.y_t && p1.y_t < WIN_HEIGHT)
